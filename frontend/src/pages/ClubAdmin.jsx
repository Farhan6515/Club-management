...
import {
  ArrowLeft, Plus, Send, UserMinus, Users,
  Edit3, Save, X, Award, ImagePlus, Camera,
} from 'lucide-react';
...
const ClubAdmin = () => {
  ...
  const [activityForm, setActivityForm] = useState({
    type: 'announcement', title: '', content: '', eventDate: '', eventLocation: '',
  });
  ...
  const handleCoverChange = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post(`/clubs/${id}/cover`, formData, { headers: { 'Content-Type': undefined } });
  ...
  if (!club) return <div className="card p-10 text-center text-gray-500 dark:text-slate-500">Club not found</div>;

  return (
    <div className="space-y-6">
      <Link to="/admin" className="inline-flex items-center gap-1 text-sm font-medium text-gray-500 hover:text-gray-900 transition dark:text-slate-400 dark:hover:text-white">
          ...
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1">
              {editingClub ? (
                ...
                <>
                  <h1 className="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">{club.name}</h1>
            ...
            <div className="flex flex-shrink-0 gap-2">
              {editingClub ? (
                <>
                  <button onClick={() => setEditingClub(false)} className="btn-secondary"><X size={14} /></button>
                  <button onClick={handleSaveClub} disabled={savingClub} className="btn-primary">
                    <Save size={14} /> {savingClub ? 'Saving…' : 'Save'}
                  </button>
                </>
              ) : (
                <button onClick={() => setEditingClub(true)} className="btn-secondary">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2">
          ...
          {showActivityForm && (
            <form onSubmit={handlePostActivity} className="card mb-4 space-y-3 p-5">
              ...
              <input type="text" required placeholder="Title" value={activityForm.title}
                onChange={(e) => setActivityForm({ ...activityForm, title: e.target.value })}
                className="input-field" />

              <textarea required rows={4} placeholder="What's the news?" value={activityForm.content}
                onChange={(e) => setActivityForm({ ...activityForm, content: e.target.value })}
                className="input-field resize-none" />

              {activityForm.type === 'event' && (
                <div className="grid gap-3 sm:grid-cols-2">
                  <input type="datetime-local" required value={activityForm.eventDate}
                    onChange={(e) => setActivityForm({ ...activityForm, eventDate: e.target.value })}
                    className="input-field" />
                  <input type="text" placeholder="Location" value={activityForm.eventLocation}
          ...
          {activities.length === 0 ? (
            <div className="card p-10 text-center text-gray-400 dark:text-slate-500">No activities yet. Post the first one!</div>
          ) : (
            <div className="space-y-4">
              {activities.map((a) => (
                <ActivityCard key={a._id} activity={a} onDelete={handleActivityDelete} />
              ))}
            </div>
          )}
        </div>
          ...
          <div className="card p-5">
            ...
            <div className="space-y-3">
              {club.members?.map((m) => {
                ...
                return (
                  <div key={m._id} className="rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-slate-700 dark:bg-slate-800/50">
                    <div className="flex items-center justify-between gap-2">
                      <div className="flex items-center gap-2 min-w-0">
                        <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-indigo-100 text-sm font-semibold text-indigo-600 overflow-hidden dark:bg-indigo-500/20 dark:text-indigo-400">
                          {m.avatar
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="truncate text-xs text-gray-400 dark:text-slate-500">{m.department}</p>
                        </div>
                      </div>
                      {!isClubAdmin && (
                        <button
                          onClick={() => handleRemoveMember(m._id, m.name)}
                          className="shrink-0 rounded-lg p-1.5 text-gray-400 hover:bg-red-50 hover:text-red-500 transition dark:text-slate-500 dark:hover:bg-red-500/10 dark:hover:text-red-400"
                          title="Remove member"
                        >
                          <UserMinus size={14} />
                        </button>
                    ...
                    <div className="mt-2.5 flex items-center gap-2">
                      <Award size={13} className="shrink-0 text-amber-500 dark:text-amber-400" />
                      {isClubAdmin ? (
                        <span className="text-xs font-semibold text-amber-600 dark:text-amber-400">Club Admin</span>
                      ) : (
                        <select
                          value={currentPos}
                          onChange={e => handlePositionChange(m._id, e.target.value)}
                          className="flex-1 rounded-lg border border-gray-200 bg-white px-2 py-1 text-xs text-gray-700 focus:border-indigo-400 focus:outline-none dark:border-slate-600 dark:bg-slate-700 dark:text-white dark:focus:border-indigo-500"
                        >
