...
import {
  Users, UserPlus, UserMinus, Settings, Tag, Calendar,
  MessageSquare, Newspaper, Plus, Send, X, ImagePlus,
} from 'lucide-react';
...
const ClubDetails = () => {
  ...
  const handleMediaChange = (e) => {
    ...
    if (files.length + mediaFiles.length > 5) { toast.error('Max 5 files per post'); return; }
  ...
  const handlePost = async (e) => {
    ...
    try {
      ...
      const { data } = await api.post(`/activities/club/${id}`, formData, { headers: { 'Content-Type': undefined } });
  ...
  if (loading) return (
    <div className="flex justify-center py-20">
      <div className="h-10 w-10 animate-spin rounded-full border-4 border-gray-200 border-t-indigo-500 dark:border-slate-700" />
    </div>
  );

  if (!club) return (
    <div className="card p-10 text-center">
      <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Club not found</h2>
      <Link to="/clubs" className="btn-primary mt-4">Browse clubs</Link>
    </div>
  );
  ...
  if (tab === 'chat' && isMember) {
    return (
      <div className="-mx-4 -mt-6 -mb-6 flex flex-col sm:-mx-6 lg:-mx-8"
        ...
        <div className="flex shrink-0 items-center gap-3 border-b border-gray-200 bg-white px-4 py-2 dark:border-slate-700 dark:bg-slate-900">
          <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-indigo-100 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400">
            <MessageSquare size={16} />
          </div>
          <span className="flex-1 truncate text-sm font-bold text-gray-900 dark:text-white">{club.name}</span>
          <div className="flex gap-1 rounded-lg border border-gray-200 bg-gray-100 p-0.5 dark:border-slate-700 dark:bg-slate-800">
            <button onClick={() => setTab('activity')}
              className="flex items-center gap-1.5 rounded px-3 py-1.5 text-xs font-semibold text-gray-500 transition hover:text-gray-900 dark:text-slate-400 dark:hover:text-white">
              <Newspaper size={13} /> Activity
            </button>
            <button className="flex items-center gap-1.5 rounded bg-indigo-100 px-3 py-1.5 text-xs font-semibold text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-400">
        ...
        <div className="p-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-gray-500 dark:text-slate-400">{club.description}</p>

              <div className="mt-4 flex flex-wrap gap-x-5 gap-y-2 text-sm text-gray-500 dark:text-slate-400">
                <span className="flex items-center gap-1.5">
                  <Users size={14} />
                  <strong className="text-gray-900 dark:text-white">{club.members?.length || 0}</strong> members
                </span>
                <span className="flex items-center gap-1.5">
                  <Calendar size={14} />
                  Created{' '}
                ...
                <span>Admin: <strong className="text-gray-900 dark:text-white">{club.admin?.name}</strong></span>
              </div>
            </div>
          ...
          {club.tags?.length > 0 && (
            <div className="mt-4 flex flex-wrap gap-1.5">
              {club.tags.map((tag) => (
                <span key={tag} className="badge bg-gray-100 text-gray-500 dark:bg-slate-700 dark:text-slate-400">#{tag}</span>
              ))}
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-2">
          <div className="mb-4 flex items-center justify-between">
            <div>
              ...
              {hasPosition && (
                <p className="text-xs text-indigo-600 mt-0.5 dark:text-indigo-400">{myPosition.title} · can post</p>
              )}
            </div>
            {canPost && !isAdmin && (
          ...
          {showPostForm && hasPosition && (
            <form onSubmit={handlePost} className="card mb-4 space-y-3 p-5">
              ...
              {mediaPreviews.length > 0 && (
                <div className="flex flex-wrap gap-2">
                  {mediaPreviews.map((p, i) => (
                    <div key={i} className="relative h-20 w-20 rounded-lg overflow-hidden border border-gray-200 dark:border-slate-600">
                      {p.type === 'video'
              ...
              <div className="flex items-center gap-2">
                <label className="flex cursor-pointer items-center gap-1.5 rounded-lg border border-gray-200 bg-gray-50 px-3 py-1.5 text-sm text-gray-600 hover:bg-gray-100 transition dark:border-slate-600 dark:bg-slate-700 dark:text-slate-300 dark:hover:bg-slate-600">
                  <ImagePlus size={15} />
                ...
                <span className="text-xs text-gray-400 dark:text-slate-500">Max 5 files · 50 MB each</span>
          ...
          {activities.length === 0 ? (
            <div className="card p-10 text-center text-gray-400 dark:text-slate-500">No posts yet from this club</div>
          ) : (
          ...
          <div className="card p-5">
            ...
            <div className="space-y-2">
              {club.members?.slice(0, 10).map((m) => {
                ...
                return (
                  <div key={m._id} className="flex items-center gap-3 rounded-lg p-2 hover:bg-gray-50 dark:hover:bg-slate-700/50">
                    <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-indigo-100 font-semibold text-indigo-600 overflow-hidden dark:bg-indigo-500/20 dark:text-indigo-400">
                      {m.avatar
                    ...
                    <div className="min-w-0">
                      <Link to={`/users/${m._id}`} className="text-sm font-semibold text-gray-900 hover:text-indigo-600 transition dark:text-white dark:hover:text-amber-400">
                        {m.name}
                      </Link>
                      <div className="flex flex-wrap items-center gap-1 mt-0.5">
                        {m._id === club.admin?._id && (
                          <span className="badge bg-amber-50 text-amber-600 dark:bg-amber-500/20 dark:text-amber-400">Admin</span>
                        )}
                        {pos && (
                          <span className="badge bg-indigo-50 text-indigo-600 dark:bg-indigo-500/20 dark:text-indigo-300">{pos.title}</span>
                        )}
                        {!pos && m._id !== club.admin?._id && (
                          <p className="text-xs text-gray-400 dark:text-slate-500">{m.department}</p>
                        )}
                      </div>
                    </div>
                  </div>
